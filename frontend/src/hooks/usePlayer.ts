...
export const usePlayer = create<PlayerStore>()(
    persist(
        (set, get) => ({
            ...
            setAudioRef: (ref) => {
                set({ audioRef: ref });
                // Apply persisted settings when ref is set
                if (ref) {
            ...
            next: () => {
                const { playlist, currentIndex, sleepMode, repeatMode, isShuffle } = get();
                ...
                if (isShuffle) {
                    ...
                    get().seek(0); // Replay current
                    get().resume();
                    return;
                } else if (repeatMode === 'all' && nextIndex >= playlist.length) {
                    nextIndex = 0; // Loop back
                }

                if (nextIndex < playlist.length) {
                    const nextChapter = playlist[nextIndex];
                    set({
            ...
            prev: () => {
                const { playlist, currentIndex, currentTime, audioRef, repeatMode } = get();

                // If more than 3 seconds into track, restart current
                if (currentTime > 3 || repeatMode === 'one') {
                    if (audioRef) {
                        audioRef.currentTime = 0;
                    }
                    set({ currentTime: 0 });
        ...
        }),
        {
            ...
            partialize: (state) => ({
                // Only persist user preferences, not playback state like 'isPlaying' or 'audioRef'
                volume: state.volume,
                speed: state.speed,
                isShuffle: state.isShuffle,
                repeatMode: state.repeatMode,
                // Optional: Persist last played track?
            ...
            skipHydration: true, // Handle hydration manualy if needed, or defaulting to false is fine for CSR
        }
    )
);
